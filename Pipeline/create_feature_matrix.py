...
def extract_snp_genotypes(vcf_path: str):
    """Creates a Genotype presence/absence matrix + feature metadata."""
    # load the vcf file and the sample names
    vcf = VCF(vcf_path)
    samples = list(vcf.samples)

    # inialize the 
    geno_rows = {}               # SNP feature matrix
    feature_meta = {}            # metadata per SNP feature

    # loop through each variant in the vcf
    for variant in vcf:
        chrom = variant.CHROM # chromosome
        pos = variant.POS # variant position
        ref = variant.REF # what is the reference allele for this variant
        alts = variant.ALT # what are the possible alternative alleles
        genotypes = variant.genotypes # what alternative allele each sample has

        a1_calls = [gt[0] for gt in genotypes] # keep only a1 (the first chromosome, bacteria are haploids)

        # Get the SNPEff annotation
        ann_str = variant.INFO.get("ANN")
        annotations = parse_ann(ann_str) if ann_str else []

        # rewrite the annotation file so that it has the allele type as index
        ann_by_alt = {}
        for ann in annotations:
        alt_allele = ann["allele"]
        ann_by_alt.setdefault(alt_allele, []).append(ann)

        # create a feature for each alternative allele
        # loop through each possible alternative allele and see if the sample has it. 
        # return -1 if the allele is absent from that sample, 0 if the sample has the reference allele and 1 if the sample has the alternative allele
        for alt_idx, alt in enumerate(alts):
            ...
            # --------------------------------
            # 2. Create feature metadata file
            # --------------------------------
            ...
            if ann_list:
                # choose strongest effect for THIS allele
...
def main():
    ...
    snp_df = extract_snp_features(args.snp)

    print("Extracting CNV features...")
    cnv_df = extract_cnv_features(args.cnv)

    # Align on shared samples (inner join; warn about mismatches)
    ...
    print(f"Shape: {matrix.shape[0]} samples × {matrix.shape[1]} features")
