...
------------------------
Creates three feature matrices for the SNPs, Structural variants, and Copy Number Variation features. 
The three feature matrices will then be used for machine learning.

Features extracted:
- SNPs: Individual variants with HIGH/MODERATE/MODIFIER impact (excludes LOW) generated by the snakemake pipeline
- Structural variants: TD, INV, LI and D types. The presence/absence and counts for each variant type are exported.
- Copy Number Variations: Depth-based copy-number variations per genomic region
